...
\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/climate_scenarios_k1x_burnin/figures/report_plots/fig01_thermal_niches.png}
\caption{\textbf{Thermal niches against baseline water temperature (single panel).} \textit{Grey bars:} relative frequency of the 2026 baseline water temperature across all 775 sampling sites; each bar is 1~$^\circ$C wide and the counts are divided by the largest count, so the tallest bar equals 1. \textit{Coloured curves:} the Gaussian thermal-suitability function $W=\exp[-(T-T_s^*)^2/(2\sigma_s^2)]$ for the native species; species with identical empirical temperature ranges are drawn once and listed together. In the legend each colour gives a species code and its thermal optimum $T_s^*$; because $\sigma_s$ is one-sixth of the empirical range, suitability falls to $\exp(-9/2)\approx0.011$ at the range edges. \textit{Vertical lines:} dotted black is the basin-mean baseline temperature (16.0~$^\circ$C); dotted crimson is the same mean after the median imposed warming to 2045 ($+0.81$~$^\circ$C, i.e.\ 16.8~$^\circ$C). \textit{Reading:} the dominant cyprinids peak at 19--20~$^\circ$C, warm of the present mean, so modest warming moves them towards their optima, whereas brown trout (\textit{Salmo trutta}, ST, optimum 12.0~$^\circ$C) is already about 1.5 thermal standard deviations (about 1.9~$^\circ$C) on the warm side of its optimum. The single panel is not lettered.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/climate_scenarios_k1x_burnin/figures/summary_2045.png}
\caption{\textbf{Final-year (2045) climate-ensemble summary by metric and spatial reporting level.} \textit{Panels:} (a)--(e) are one metric each, labelled on the y-axis with its units: (a) native richness (species per site); (b) invasive richness (species per site); (c) realised native richness loss (fraction of the spun-up baseline); (d) total biomass (model density units per site); (e) projected water temperature ($^\circ$C). The four spatial levels are no longer repeated as separate panels; they are the \textit{colour} inside each panel: blue = sampling point, green = sub-basin, orange = water body, red = whole basin (legend, bottom right). \textit{x-axis:} emission pathway, including the no-warming control. \textit{Points:} mean across the 11 GCMs of that level's mean across its units. \textit{Bars:} thin = 10--90\% and thick = 25--75\% spread across GCMs. Merging the four levels into one panel per metric gives them a common y-axis, which is what makes the level-to-level and pathway-to-pathway comparisons direct. \textit{Two nearly flat panels are expected rather than a plotting error:} richness is a threshold count (a species is counted when its density exceeds 0.1). Invasive richness is exactly 0.2168 species per site (168 of the $775\times10$ invasive site--species combinations) under every scenario and under the control, because no invasive population crosses the 0.1 threshold during 2026--2045; invasive \emph{biomass} does change. Across the 44 climate runs only temperature, and weakly total biomass, separate the pathways; aggregate richness and biomass overlap strongly and the control lies within the scenario spread.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/climate_scenarios_k1x_burnin/figures/comparison/across_scenarios_basin.png}
\caption{\textbf{Annual whole-basin trajectories across emission pathways, 2026--2045.} \textit{Panels:} (a) native richness (species per site); (b) invasive richness (species per site); (c) realised native richness loss (fraction); (d) total biomass (model density units per site); (e) projected water temperature ($^\circ$C). \textit{Lines:} mean across the 11 GCMs of the whole-basin value for each pathway. \textit{Bands:} 10--90\% spread across GCMs. \textit{Colour and legend} (outside the panels, bottom right): blue = SSP1-2.6, green = SSP2-4.5, orange = SSP3-7.0, red = SSP5-8.5, black = no-warming control. \textit{How this differs from the forcing-versus-response figure below:} this figure is purely temporal and uses \emph{absolute} water temperature; the forcing-versus-response figure isolates the \emph{warming anomaly} and pairs each trajectory with a final-year response relationship. The temperature trajectories diverge as expected, but native richness, realised richness loss and total biomass remain nearly coincident across pathways, and the control is nearly stationary (its small residual drift is an internal cycle below about 1\%). The invasive-richness panel is a flat line at 0.2168 for the threshold-count reason given in the final-year summary figure. The temporal persistence of this pattern supports interpreting the flat aggregate climate slopes as a model result rather than incomplete equilibration.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.62\textheight,keepaspectratio]{../results/climate_scenarios_k1x_burnin/figures/diagnostics/forcing_and_response.png}
\caption{\textbf{Forcing versus response.} ``Forcing'' is the imposed basin-mean water-temperature anomaly $\Delta T$ relative to the 1986--2005 baseline, i.e.\ the warming the climate ensemble adds on top of the observed seasonal climatology; it is shown explicitly because the other figures plot either absolute temperature (the whole-basin trajectory figure) or only the response. \textit{Top row (trajectories, median across the 11 GCMs; band = 10--90\% across GCMs):} (a) forcing $\Delta T$ ($^\circ$C) over time, with the four SSPs separating; (b) native richness (species per site); (c) total biomass (model density units per site). The annotations ``all SSPs overlap'' in (b) and (c) flag that the response trajectories lie on top of one another even though the forcing separates. \textit{Bottom row (one point per GCM~$\times$~SSP run, coloured by pathway):} (d) final-year native richness, (e) final-year total biomass and (f) final-year relative native richness loss, each against the \emph{realised} warming of that run. The realised anomaly is used here because it is the quantity that actually drives the biology in each run; it differs by GCM from the imposed basin-mean axis used for the ordinary-least-squares slopes in the dose--response figure below. The Pearson correlations quoted in (d)--(f) are computed across the 44 SSP runs; the no-warming control is plotted for reference but excluded from the correlation, matching the dose--response analysis elsewhere in the report. The loss metric is a thresholded realised loss, not an extinction probability. The point of the figure is that the model transmits forcing clearly while the aggregate biological response remains weak.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_temperature_response.png}
\caption{\textbf{Basin response versus warming across the 44-run climate ensemble (2026--2045, not end-of-century).} \textit{Panels:} (a) final basin total biomass; (b) final basin native richness; (c) realised native richness loss. \textit{Why there are four points per pathway:} each point is one of the 11 GCMs under one of the four SSPs, giving $11\times4=44$ points in total; the colour identifies the SSP (legend below the panels). \textit{x-axis:} the imposed basin-mean warming at 2045 (relative to 1986--2005), spanning about 0.28--1.37~$^\circ$C. \textit{Quantities shown:} (a) model density units per site, (b) species per site, (c) fraction of spun-up richness. \textit{Dashed black line:} ordinary least-squares fit across the 44 points; the inset gives its slope and $R^2$. The fitted slopes are $-0.120$ model units per $^\circ$C ($R^2=0.25$), $-0.00071$ species per site per $^\circ$C ($R^2=0.02$) and $+0.00015$ loss units per $^\circ$C ($R^2=0.01$). \textit{Magenta stars:} the two runs selected as the cool endpoint (SSP1-2.6/IITM-ESM) and warm endpoint (SSP2-4.5/UKESM1) of the obstacle experiment. The heavy overlap of pathway colours explains why no emission-pathway ranking is defensible over this horizon.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.55\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_parameter_importance_ranked.png}
\caption{\textbf{Factor importance and how it is computed.} \textit{Panel (a):} overall ranking of the seven factors. The x-axis is the mean \emph{normalised} effect (1.0 = the largest effect for that metric); the annotation gives the mean rank and the mean normalised effect. \textit{Panel (b):} the per-metric normalised effect size as a heatmap; rows are the eight response metrics, columns the seven factors, the number in each cell is the normalised effect and the colour scale is shown by the colourbar. \textit{Computation:} within each matched factorial block (all other design columns held fixed) the effect span is the difference between the largest and smallest metric value across the levels of one factor; spans are standardised by the standard deviation of that metric across all runs in the sweep, the median span across blocks is taken, and for each metric the medians are divided by the largest so that the most important factor equals 1.0; the rank of a factor is its position within each metric (1 = most important), averaged across the eight metrics. \textit{Factor meanings:} passability = the four obstacle-overlay states; upstream cost = the four elevation-cost settings; climate endpoint = the cool versus warm sensitivity end (only two levels, an external reference axis rather than a full climate-model sample); interaction matrix = original, random placebo and invasive-favouring; thermal breadth = the $\sigma$ multiplier, confounded with the alternative sweep and its shorter burn-in; the last two columns are the passability~$\times$~upstream-cost and passability~$\times$~temperature interactions. Interaction structure ranks first (mean rank 1.88; mean normalised effect 0.871), followed by upstream cost, thermal breadth, passability, the passability--cost interaction, the climate endpoint and the passability--temperature interaction. The apparent importance of passability for brown-trout quasi-extinction is inflated by the near-saturated threshold diagnostic.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_tornado.png}
\caption{\textbf{Signed factor effects (tornado), one panel per response metric.} \textit{Panels:} (a) total biomass, (b) native biomass, (c) invasive biomass, (d) native richness, (e) realised native richness loss, (f) brown-trout relative biomass change, (g) brown-trout final biomass and (h) brown-trout quasi-extinct fraction. \textit{Each bar:} the median over matched factorial blocks of the signed difference between one factor level and its reference level, divided by the standard deviation of that metric across the relevant run set, so all bars are in comparable ``$\sigma$ units''; a bar to the left means the level lowers the metric. \textit{Bar labels:} every label names the level contrast explicitly, for example ``Blocked $-$ baseline'' (passability), ``Cost 0.50 $-$ 0.01'' (upstream movement cost), ``Invasive $-$ original'' (interaction matrix), ``Warm $-$ cool endpoint'' (climate) and ``Breadth $\times0.3$ $-$ $\times1.0$'' (thermal breadth); the factor that each contrast belongs to is given by the colour (legend at right). Reference levels are upstream cost 0.01, baseline passability, the original interaction matrix, the cool climate endpoint and breadth multiplier 1.0. \textit{Colour/legend} (right): blue = passability states; orange = upstream cost; red = climate endpoint; green = interaction matrix; purple = thermal breadth. \textit{Reading:} increasing upstream cost generally reduces biomass and richness; passability effects change sign with the background cost; the invasive-favouring matrix has a much larger and more asymmetric effect than the random placebo; the downward thermal-breadth effect is shown for completeness but is confounded with the alternative sweep and its shorter burn-in.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_passability_uc_heatmaps.png}
\caption{\textbf{Passability by upstream-cost interaction (obstacle sweep).} \textit{Panels:} (a)--(h); columns are the two climate endpoints (left: cool, SSP1-2.6/IITM-ESM; right: warm, SSP2-4.5/UKESM1) and rows are four response metrics, labelled on the left: total biomass, native biomass and native richness (all in model density units per site except richness, which is species per site), and brown-trout (ST) final biomass (model units). \textit{Axes:} x = the four upstream movement-cost settings (0.01, 0.05, 0.10, 0.50); y = the four obstacle-overlay passability states (baseline, improved, reduced, blocked). \textit{Cell value:} the raw change of the metric relative to the same-model, same-cost baseline-passability run (so the baseline row is exactly 0 everywhere); units are the model units of the metric. \textit{Cell colour:} the same change divided by that panel's 95th percentile of absolute changes, so colours are comparable within a panel but not between panels (the colourbar spans $-1$ to $+1$; white = no change). \textit{Reading:} for native biomass the blocked-minus-baseline change goes from $-0.349$ at cost 0.01 to $+0.442$ at cost 0.50 at the cool endpoint, and the warm endpoint shows the same reversal ($-0.336$ to $+0.453$); reduced passability follows the same pattern, whereas improved passability is negative at every cost. The sign reversal is context dependence; it does not identify barrier construction as a conservation action. The design cannot by itself separate reduced native rescue from suppressed invasive dispersal.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_st_vs_upstream_cost.png}
\caption{\textbf{Brown trout response to hydraulic structure (four equal-size panels).} \textit{Panels:} (a) and (c) are the cool endpoint (SSP1-2.6/IITM-ESM); (b) and (d) are the warm endpoint (SSP2-4.5/UKESM1). The top row is final \textit{Salmo trutta} (ST, brown trout) biomass (model units) and the bottom row is ST relative biomass change (fraction of the spun-up baseline). \textit{x-axis:} the four categorical upstream movement costs (0.01, 0.05, 0.10, 0.50). \textit{Coloured lines/points:} the four passability states; the legend (below the panels) is black = baseline ($\times1.0$), blue = improved ($\times1.5$), orange = reduced ($\times0.5$), red = blocked ($\times0.1$). \textit{Reading:} all passability curves lie below the spun-up state and the warm endpoint is uniformly more negative; at cost 0.05 the baseline-to-blocked change is 803.9 to 770.9 units at the cool endpoint and 725.0 to 704.2 units at the warm endpoint, so a climate penalty is superimposed on a smaller but non-negligible hydraulic penalty.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_interaction_matrix_robustness.png}
\caption{\textbf{Passability robustness across interaction matrices (four panels).} \textit{Panels:} (a) and (c) are the cool endpoint; (b) and (d) the warm endpoint (column headers). The top row is brown trout (\textit{Salmo trutta}, ST) final biomass and the bottom row is basin native biomass (all ten native species pooled), both in model units, labelled on the left. \textit{y-axis:} the three interaction matrices (original, random placebo, invasive-favouring). \textit{What each bar is:} the \emph{span across passability} is the difference between the largest and smallest value of the metric when the four passability states (baseline, improved, reduced, blocked) are compared at the same matrix, climate endpoint and cost; a long bar means passability strongly controls the metric. \textit{Why there are two bars per matrix:} each interaction matrix was run at two upstream-cost settings, low (0.01) and high (0.50), drawn as the two bars of each pair; blue bars are the low-cost setting and orange bars the high-cost setting. The panel deliberately carries no legend. \textit{Reading:} ST spans are largest under the invasive-favouring matrix (mean about 23.8 units, versus 10.2 under the original matrix and 8.5 under the random placebo), whereas native-biomass spans are smallest under the invasive-favouring matrix (about 0.2 versus 1.3 and 1.4 units) because native biomass has already collapsed. The figure is a structural-uncertainty warning: both the magnitude and, in this confounded sweep, the sign of passability effects depend on the interaction assumptions.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/report_plots/fig_subcatchment_waterbody_effects.png}
\caption{\textbf{Spatial distribution of blockage effects.} \textit{Scenario:} warm endpoint, high upstream cost (0.50), year 2045; blocked minus baseline passability. \textit{Panel (a):} the 10 most negative and 10 most positive of the 77 sub-catchments (20 bars); y labels are sub-catchment codes. \textit{Panel (b):} the 10 most negative and 10 most positive of the 289 water bodies (20 bars); y labels are water-body codes, truncated for display. Both panels are the same size, use the same x-axis (change in native biomass, model units) and show bars sorted from most negative to most positive; red bars are losses and green bars are gains. The two panels show only the extremes for legibility; the full distributions are summarised in the text. \textit{Message:} the positive basin-level blocked-minus-baseline effect is a net of local gains and losses, so it must not be read as a uniform benefit of blocking. The sub-catchment or water body is the appropriate scale for management decisions.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/climate_scenarios_k1x_burnin/figures/report_plots/fig12_st_climate_response.png}
\caption{\textbf{Brown trout climate dose--response.} \textit{Panels:} (a) relative change in basin \textit{Salmo trutta} (ST) biomass (\% of the spun-up baseline); (b) absolute final ST biomass (model units); (c) maximum number of days per year above the empirical 20~$^\circ$C upper thermal limit. \textit{Points:} one per GCM~$\times$~SSP run (44 in total), coloured by pathway; the black diamond at 0~$^\circ$C is the no-warming control. \textit{x-axis:} imposed basin-mean warming at 2045. \textit{Dashed line in (a):} ordinary least-squares fit, giving a decline of about 9.2\% of baseline per $^\circ$C ($R^2=0.65$); the control changes by only $-0.2$\%. (b) shows the absolute decline behind that ratio, and (c) shows the physiological exposure mechanism: maximum annual days above 20~$^\circ$C rise from 131 in the control (dotted reference line) to 139--174 across scenarios. The figure links a species-level abundance response to a thermal-exposure mechanism while showing why it should not be extrapolated to the whole assemblage.}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.98\textwidth,height=0.65\textheight,keepaspectratio]{../results/sensitivity_obstacles/alt_interactions/report_plots/fig13_interaction_matrix_effects.png}
\caption{\textbf{Interaction-matrix main effects (four panels).} \textit{Panels:} (a) basin native biomass (all native species pooled); (b) basin invasive biomass; (c) brown trout (\textit{Salmo trutta}, ST) final biomass; (d) basin native richness (species per site). All biomass panels are in model units; the y-axis label gives the units. \textit{x-axis:} the three interaction assumptions, original qualitative matrix, random placebo matrix and invasive-favouring matrix. \textit{Points:} every one of the 16 runs per matrix (two climate endpoints $\times$ two upstream costs $\times$ four passability states); a small horizontal jitter separates overlapping points. \textit{Black horizontal line:} the matrix mean across those 16 runs. \textit{Annotations:} the mean original~$\rightarrow$~invasive-favouring change with its percentage. Relative to the original matrix, the invasive-favouring matrix shifts native biomass from about 50.2 to 5.1 units ($-90$\%), ST biomass from 548 to 313 ($-43$\%), native richness from 2.4 to 0.1 ($-94$\%) and invasive biomass from 6.0 to 58.7 (+881\%), while total biomass changes comparatively little (56.2 to 64.2; not shown here). The random-matrix means are not ecologically interpretable because some species have near-zero burn-in baselines. The figure therefore shows redistribution between community compartments, but the deliberately extreme matrix, the confounded thermal-breadth multiplier and the shorter burn-in make it a bounding scenario rather than a forecast.}
\end{figure}
