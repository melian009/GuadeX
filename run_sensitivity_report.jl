...
# =============================================================================
# --- Obstacle / upstream-cost sensitivity: passability x dispersal cost ---
#
# Climate-consistent with `run_climate_scenarios.jl` / the
# `climate_scenarios_k1x_burnin` ensemble: per-site daily water temperature from
# two extreme climate projections, a seasonal baseline burn-in, calibrated heat
# stress, the WP0 species classification (ST native, migratory group separate)
# and K = 1x observed.  The swept axes are the upstream dispersal cost and the
# per-subcatchment obstacle/passability scenario.
#
# Settings come from `[obstacle_sensitivity]` and `[run_sensitivity_report]` in
# parameters.toml (see docs/climate_scenarios.md).
# =============================================================================
...
SimulationParameters.require_sections(GUADEX_PARAMS, "general", "inputs", "obstacles",
    "species", "subcatchments", "scenarios", "obstacle_sensitivity", "run_sensitivity_report")
...
# --- Parameter grid ---
const UPSTREAM_COSTS = SimulationParameters.float_vector(GUADEX_PARAMS["run_sensitivity_report"]["upstream_costs"])
const PASSABILITY_NAMES = String.(GUADEX_PARAMS["run_sensitivity_report"]["passability_scenarios"])
...
println("="^70)
println("Obstacle / upstream-cost sensitivity (daily forcing + burn-in)")
...
# =============================================================================
# --- Base data, optimisation, heat stress and burn-in (once) ---
# =============================================================================
...
if SETTINGS.thermal_optima_fraction != 0.5
    ...
    println("WP3 optimum sweep: fraction=$(SETTINGS.thermal_optima_fraction)")
end
...
# WP1: ensemble-median historical daily series, used for the burn-in and the
# heat-stress calibration (the per-GCM runs difference against their own
# historical, exactly as in the climate-scenario runner).
...
params = set_heat_stress_rate(base_params, k_heat)
println("Heat-stress mortality: $(k_heat > 0 ? "k=$k_heat" : "disabled")")

spin = if SETTINGS.spin_up
    get_or_compute_spinup(SETTINGS, params, vec(u0_obs), baseline_temps, baseline_dates;
        tag="sensitivity_report",
        extra="mode=$(OBSTACLE_MODE)_tol=$(OBSTACLE_MATCHING_TOLERANCE)_" *
              "pass=$(OBSTACLE_PASSABILITY)_down=$(OBSTACLE_DOWNSTREAM_PASSABILITY)_" *
              "obs=$(basename(OBSTACLES_FILE))")
else
    println("Burn-in disabled ([obstacle_sensitivity].spin_up = false); " *
            "starting from observed densities")
...
index_df = read_sensitivity_index(index_path)

total_runs = length(CLIMATE_MODELS) * length(UPSTREAM_COSTS) * length(PASSABILITY_NAMES)
current_run = 0
started = time()
...
for model in CLIMATE_MODELS
    ...
    println("\n" * "-"^70)
    println("Climate model: $(model.scenario) / $(model.gcm)")
    println("-"^70)
    ...
    for uc in UPSTREAM_COSTS
        for pass_name in PASSABILITY_NAMES
            ...
            run_dir = joinpath(base_output_dir, model_tag, "uc_$(uc)", pass_name)
            run_label = "$model_tag uc=$(uc) pass=$(pass_name)"
...
println("\n" * "="^70)
println("Obstacle sensitivity complete: $(nrow(index_df)) run(s) under '$base_output_dir' in $(elapsed)s")
