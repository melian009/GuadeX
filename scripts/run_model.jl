...
# Management scenarios (one exploitation and one passability scenario).
all_subcatchments = SimulationParameters.float_vector(GUADEX_PARAMS["subcatchments"]["all"])
example_exploitation_scenario = SimulationParameters.scenario_dict(
    all_subcatchments, GUADEX_PARAMS["scenarios"]["exploitation"][GUADEX_PARAMS["run_model"]["exploitation_scenario"]])
example_passability_scenario = SimulationParameters.scenario_dict(
    all_subcatchments, GUADEX_PARAMS["scenarios"]["passability"][GUADEX_PARAMS["run_model"]["passability_scenario"]])
...
# The exploitation and passability scenarios are read from parameters.toml.
# =============================================================================
...
jldsave(output_jld2;
    ...
    obstacle_passability = OBSTACLE_PASSABILITY,
    obstacle_downstream_passability = OBSTACLE_DOWNSTREAM_PASSABILITY,
    obstacle_matched_count = count(data.obstacle_mapping_diagnostics.matched),
